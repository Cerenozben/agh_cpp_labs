...
#include <Classes/Factory/ProductFactory.hpp>
...
{
protected:
    warehouse::ProductFactory factory_{};
};
...
TEST_P(ProductFactoryTest, CreateProduct)
{
    ...
    classesInterface::IProductPtr productPtr = factory_.createProduct(className, name, size);
...
{
    EXPECT_THROW(factory_.createProduct("Unknown class", "nope", -1.0), std::runtime_error);
}
